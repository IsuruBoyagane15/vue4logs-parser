...
def get_bigrams(doc):
    ...
    if doc_length % 2 != 0:
        print("boo")
        doc.append("FILLER_TOKEN")
    print(doc)
    for i in range(0,doc_length,2):
        print(i, i+1)
        bigrmas.append(" ".join(doc[i:i+2]))
    return bigrmas
...
class Vue4Logs:
    def __init__(self, threshold, dataset):
        ...
        self.output_path = "results_bi/" + str(threshold)
    ...
    def index_doc(self, doc_id):
        ...
        bi_new_t = get_bigrams(new_template)
    ...
    def parse(self):
        ...
        for idx, line in df_log.iterrows():
            ...
            lengt = len(pre_processed_log)
            pre_processed_log = get_bigrams(pre_processed_log)
